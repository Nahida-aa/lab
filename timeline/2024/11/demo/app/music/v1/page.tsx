...
const Stars = () => {
  ...
  return (
    <group>
      {stars.map((pos, index) => (
        <Star key={index} position={pos as [number, number, number]} />
      ))}
    </group>
  )
}
...
const Meteor = () => {
  ...
  const [position, setPosition] = useState(new THREE.Vector3(Math.random() * 100 - 50, 50, Math.random() * 50 - 25))

  useFrame(() => {
    if (meshRef.current) {
      meshRef.current.position.x -= 1
      meshRef.current.position.y -= 0.5
      setTrail(prevTrail => [...prevTrail, meshRef.current!.position.clone()].slice(-20))
      if (meshRef.current.position.x < -50 || meshRef.current.position.y < -50) {
        setPosition(new THREE.Vector3(Math.random() * 100 - 50, 50, Math.random() * 50 - 25))
        setTrail([])
      }
    }
  })
    ...
    <group>
      ...
      {trail.map((pos, index) => (
        <mesh key={index} position={pos}>
          <sphereGeometry args={[0.1 * (1 - index / trail.length), 8, 8]} />
          <meshBasicMaterial color="#FFFFFF" transparent opacity={1 - index / trail.length} />
        </mesh>
      ))}
    </group>
  )
...
export default function MusicPlayer() {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen bg-black text-white p-4 overflow-hidden">
      ...
      <div className="z-10 w-full max-w-md bg-white/10 backdrop-blur-lg rounded-lg shadow-lg overflow-hidden">
