...
const use3DEffect = (isHovering: boolean) => {
  ...
  const transform = useMemo(() => {
    ...
    const maxRotation = 10 // 减小最大旋转角度
    const maxDepression = 5 // 减小最大压力效果

    // 使用非线性函数计算旋转角度
    const calculateRotation = (value: number, max: number) => {
      const normalized = value / max
      return Math.atan(normalized * 2) / (Math.PI / 2) * max
    }

    const rotateY = calculateRotation(mousePosition.x - centerX, centerX) * maxRotation
    const rotateX = calculateRotation(mousePosition.y - centerY, centerY) * -maxRotation

    const distanceFromCenter = Math.sqrt(
      Math.pow(mousePosition.x - centerX, 2) + Math.pow(mousePosition.y - centerY, 2)
    )
    const maxDistance = Math.sqrt(Math.pow(centerX, 2) + Math.pow(centerY, 2))
    const depression = (1 - Math.min(distanceFromCenter / maxDistance, 1)) * maxDepression
