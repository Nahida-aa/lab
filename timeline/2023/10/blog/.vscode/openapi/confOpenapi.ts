...
export default async function configOpenAPI(app: AppOpenAPI) {
  app.doc31("/openapi.json", (c) => ({
    ...
    info: {
      ...
      description: `- [swagger-ui](/api/doc/swagger)\n- [scalar-ui](/api/doc/scalar)\n- [index](/)\n- [auth doc](/api/auth/reference)\n - serverUrl: ${appUrl || new URL(c.req.url).origin}\n - https://local.drizzle.studio
      `,
    },
    servers: [
      {
        url: appUrl || new URL(c.req.url).origin,
  ...
  const openapiScalarPath = "/doc/scalar";
  app.get(
    openapiScalarPath,
    Scalar({
  ...
  console.log(`Scalar UI is available at ${serverUrlPath}${openapiScalarPath}`);
