...
function createTrail(options) {
  // console.log("create trail",options)
...
async function createCursorHandler(handlerFunctions, editorContainerId) {
  // Get Editor with dirty way (... due to vscode plugin api's limit)
  /** @type { Element } */
  let editor
  while (!editor) {
    await new Promise(resolve=>setTimeout(resolve, 100))
    // editor = document.querySelector(".part.editor") // vscode
    editor = document.querySelector(".monaco-editor") // monaco
    // editor = document.querySelector(`#${editorContainerId} .monaco-editor`)
  }
  ...
  function createCursorUpdateHandler(target,cursorId,cursorHolder,minimap) {
    // console.log("createCursorUpdateHandler",cursorId)
    let lastX,lastY // save last position
    let update = (editorX,editorY)=>{
      // console.log("update",cursorId)
...
// console.log("Main handler code")
// Main handler code
let cursorCanvas,rainbowCursorHandle
// console.log("createCursorHandler")
createCursorHandler({
