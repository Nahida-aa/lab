...
* 作为鉴赏家，我希望通过评论和点赞获得积分，逐步升级身份，获得更多特权。
...
* 系统记录用户行为（评论、点赞、下载、投资、施工贡献）并计算积分/贡献值。
* 用户可查看当前身份等级及升级条件。
* 升级成功自动获得新权限和徽章。

#### 3.3.2 项目社群

* 每个作品自动关联一个 Discord 社群或内置聊天室。
* 创作者管理社群，发布公告、更新日志。
...
Cookie: better-auth.session_token=TI7O2LOXQcmh8wrFZaCs40PhuNqmGzJh.q4XY3Ljzz4wz1We47SNX1FFyTmRIiOxD5iYB8JE7Z%2F4%3D
...
```
创建 project , 将会自动创建 community, 多个 channel 
...
```http
POST /api/project/version
Content-Type: application/json

{
  "projectId": "xxx-uuid",
  "versionNumber": "1.0.0",
  "isPrimary": true,      // 至少要有一个 primary version
  "files": [
    {
      "name": "example.zip",
...
* **注意**：
  ...
  * `Content-Type` 必须与创建版本时指定的 `file.type` 一致。
  * 预签名 URL 通常有时效限制，请在有效期内完成上传。

##### **3. 标记上传完成**

上传成功后，需要调用 **PATCH 接口**，告知系统文件已上传完成：

**接口**

```http
PATCH /api/project/version/{versionId}
Content-Type: application/json
...
1. POST `/project/version` → 创建版本记录 + 获取上传 URL
2. PUT 预签名 URL → 上传文件到 OSS/S3
3. PATCH `/project/version/{versionId}` → 标记文件上传完成


#### 3.3.3 投资与奖励

* 投资者使用平台积分/虚拟货币支持作品或创作者。
* 系统记录投资额并展示在项目主页。
...
* **用户表**：ID、昵称、身份等级、积分、投资额、施工贡献、注册时间。
...
* **性能**：动态流、社群入口加载 ≤ 300ms；身份升级奖励实时生效。
...
* **依赖**：积分系统、Discord API 或内置聊天室、内容审核机制。
