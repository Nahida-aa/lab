...
export const Header = ({ className = '' }: { className?: string }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollTop > lastScrollTop) {
        // 如果现在距离顶部的位置大于上一次距离顶部的位置那么, 说明在向下滚动
  ...
  return (
    <motion.header
      className={`px-2 md:px-4  rounded-md mx-auto bg-background/0 backdrop-blur supports-[backdrop-filter]:bg-background/10 ${isScrolled ? 'shadow-md' : ''} dark:shadow-gray-800 ${siteMetadata.stickyNav ? 'sticky  top-0 z-50' : ''} 
  ${className}`}
