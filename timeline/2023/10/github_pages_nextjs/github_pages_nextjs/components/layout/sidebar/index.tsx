...
import { TooltipX } from '@/components/uix/tooltip'
...
export function AppSidebar({ ...props }: AppSidebarProps) {
  ...
  return (
    <Sidebar
      ...
      className="Sidebar bg-transparent justify-between h-screen"
    >
      <SidebarHeader>
        <SideSwitcher list={sides} defaultItem={sides[0]} />
        {/* <SearchForm /> */}
      </SidebarHeader>
      <ScrollShadow hideScrollBar className="h-full">
        <SidebarContent>
                  ...
                  <SidebarMenuItem key={item.label}>
                    <SidebarMenuButton asChild>
                      <Link href={item.href} title={item.label}>
                        ...
                        />
                        <span className={`${pathname === item.href ? 'gradient' : ''}`}>
                          {item.label}
                        </span>
                      </Link>
                    </SidebarMenuButton>
          ...
          <SidebarGroup>
            ...
            <SidebarGroupContent
              className={`space-y-1 space-x-1 ${open || openMobile ? '' : 'hidden'}`}
            >
              {sortedKeys.map(tag => (
                <TooltipX key={tag} content={`该标签下有${tagObjsKV[tag].count}篇文章`}>
      ...
      <SidebarFooter>
        <SidebarMenu
          className={`flex-col ${collapsible === 'none' || state === 'expanded' || openMobile ? 'flex-row' : ''} justify-between`}
        >
